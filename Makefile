...
${AF_PATH} :
	...
	${PYTHON} ${QQ_CODE_DIR}/train.py ${HIST_VAR} ${REF_VAR} $@ --hist_files ${HIST_DATA} --ref_files ${REF_DATA} --hist_time_bounds ${HIST_START} ${HIST_END} --ref_time_bounds ${REF_START} ${REF_END} --scaling ${SCALING} --nquantiles ${NQUANTILES} ${GROUPING} --input_hist_units ${HIST_UNITS} --input_ref_units ${REF_UNITS} --output_units ${OUTPUT_UNITS} ${HOUR_SELECTION} --verbose --short_history ${SSR} ${VALID_MIN} ${VALID_MAX} ${COMPRESSION}
